...
def test_create_platform_monitor_linux():
    ...
    with patch("sys.platform", "linux"):
        with patch(
            "src.infrastructure.os.platform_monitor.LinuxMonitor"
        ) as mock_monitor:
            monitor = create_platform_monitor()
...
def test_create_platform_monitor_macos():
    ...
    with patch("sys.platform", "darwin"):
        with patch(
            "src.infrastructure.os.platform_monitor.MacOSMonitor"
        ) as mock_monitor:
            monitor = create_platform_monitor()
            mock_monitor.assert_called_once()
...
def test_create_platform_monitor_unsupported():
    ...
    with patch("sys.platform", "unsupported"):
        with pytest.raises(
            NotImplementedError, match="Platform 'unsupported' is not supported"
        ):
